...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function mostrarSecao(id) {
        // Redireciona para a função no módulo Auth
        Auth.UI.mostrarSecao(id);
    }
    ...
    function fazerLogin(event) {
        // Esta função agora é tratada pelo módulo Auth
        console.warn('A função fazerLogin foi movida para o módulo Auth');
    }
    ...
    function cadastrarUsuario(event) {
        // Esta função agora é tratada pelo módulo Auth
        console.warn('A função cadastrarUsuario foi movida para o módulo Auth');
    }
    ...
    async function carregarUsuarios() {
        ...
        if (!Auth.isUserAdmin()) {
            Auth.showMessage(document.getElementById('usuarios-mensagem') || document, 
        ...
        try {
            ...
            const response = await api.listarUsuarios(currentUser.id);
            ...
            if (response.success) {
                ...
                Auth.showMessage(document.getElementById('usuarios-mensagem') || document,
                               'Erro ao carregar usuários: ' + (response.error || 'Erro desconhecido'),
                               'danger');
            }
        } catch (error) {
    ...
    async function adicionarNovoUsuario(event) {
        ...
        try {
            ...
            const response = await api.adicionarUsuario({ nome, email, senha }, currentUser.id);
    ...
    function confirmarExclusaoUsuario(usuario) {
        ...
        novoBtn.addEventListener('click', async function() {
            try {
                ...
                const response = await api.excluirUsuario(usuario.id, currentUser.id);
     ...
     */
    document.addEventListener('DOMContentLoaded', function() {
        // O módulo Auth já cuida da navegação entre login e cadastro
        
        // Event listeners existentes para navegação dentro do app
        document.getElementById('nav-eventos').addEventListener('click', function() {
            Auth.UI.mostrarSecao('eventos-section');
            carregarEventos();
        });
        
        document.getElementById('nav-fornecedores').addEventListener('click', function() {
            Auth.UI.mostrarSecao('fornecedores-section');
            carregarFornecedores();
        });
        
        document.getElementById('nav-financeiro').addEventListener('click', function() {
            Auth.UI.mostrarSecao('financeiro-section');
        ...
        document.getElementById('btn-novo-evento').addEventListener('click', function() {
            Auth.UI.mostrarSecao('novo-evento-section');
        });
        ...
        document.querySelector('#nav-usuarios').addEventListener('click', function() {
            Auth.UI.mostrarSecao('usuarios-section');
            carregarUsuarios();
        });
        
        // Funções de utilitário compartilhadas
        window.mostrarSpinner = Auth.UI.mostrarSpinner;
        window.esconderSpinner = Auth.UI.esconderSpinner;
        
        // Inicializa o sistema após a autenticação
        inicializarSistema();
    });
});